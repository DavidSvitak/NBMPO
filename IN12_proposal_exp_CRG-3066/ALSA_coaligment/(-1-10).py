...
# 168 = (002), 169 = (-1-10), 167 = (-1-1-1)

def G(x, pos, ampl, fwh):
    return ampl*np.exp(-(x-pos)**2/(2*(fwh/4.29)**2))

set_datatemplate('IN12_proposal_exp_CRG-3066/rawdata/075%03d')
data = read_data(169, 'A3P',"CNTS")
model = Background() + Gauss('peak1', pos=-47.8, ampl=3500, fwhm=3) + Gauss('peak2', pos=-50, ampl=1000,fwhm=1) + Gauss('peak3', pos=-46, ampl=1000, fwhm=1) + Gauss('peak4',pos=-52,ampl=600,fwhm=1) 
data.name = 'data'
...
with open("IN12_proposal_exp_CRG-3066/ALSA_coaligment/(-1-10)_result.txt", "a") as txt:
...
plt.savefig("IN12_proposal_exp_CRG-3066/ALSA_coaligment/(-1-10).png", dpi=600)
plt.show()
